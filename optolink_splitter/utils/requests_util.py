...
def response_to_request(
    ...
    if numelms == 1:
        ...
        data = optolinkvs2.receive_fullraw(
            settings_ini.fullraw_eot_time, settings_ini.fullraw_timeout, serViDev
        )
        ...
        if cmnd == "raw":  # "raw;4105000100F80806"
            ...
            if addr in settings_ini.w1sensors:
                ...
                # Optolink item
                retcode, addr, data = optolinkvs2.read_datapoint_ext(
                    addr, int(parts[2]), serViDev
                )
                if retcode == 1:
                    if numelms > 3:
            ...
            retcode, addr, data = optolinkvs2.write_datapoint_ext(
                utils.get_int(parts[1]), bval, serViDev
            )
            if retcode == 1:
