...
def read_datapoint_ext(addr:int, rdlen:int, ser:serial.Serial) -> tuple[int, int, bytearray]: 
    ...
    return receive_vs2telegr(True, ser)
...
def receive_vs2telegr_raw(resptelegr:bool, ser:serial.Serial, ser2:serial.Serial=None) -> tuple[int, bytearray]:
    ...
    while(True):
            ...
            if(resptelegr):
                if(len(inbuff) > 0):
                    ...
                    if(inbuff[0] == 0x06): # VS2_ACK
                        ...
                        print("NACK Error")
                        return 0x15, alldata       # hier müsste ggf noch ein eventueller Rest des Telegrams abgewartet werden 
                    else:
                        print("unknown first byte Error")
                        return 0x20, alldata
                    # erstes Byte abtrennen
        ...
        if(state == 1):
            if(len(inbuff) > 0):
                if(inbuff[0] != 0x41): # STX
                    print("STX Error")
                    return 0x41, alldata  # hier müsste ggf noch ein eventueller Rest des Telegrams abgewartet werden
                state = 2
        ...
        if(state == 2):
            if(len(inbuff) > 1):  # STX, Len
                ...
                if(len(inbuff) >= pllen+3):  # STX + Len + Payload + CRC
                    ...
                    inbuff = inbuff[:pllen+4]  # make sure no tailing trash 
                    #addr = (inbuff[4] << 1) + inbuff[5]
                    #data = inbuff[7:pllen+2]   # STX + Len + FnctCode + MsgId + AddrHi + AddrLo + BlkLen (+ Data) + CRC
                    if(inbuff[-1] != calc_crc(inbuff)):
                        print("CRC Error")
                        return 0xFE, alldata
                    #if(inbuff[2] & 0x0F == 0x03):
