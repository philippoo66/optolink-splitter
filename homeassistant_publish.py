''' 
   ...
   The configuration is defined in the ha_ha_device.py file.
...
from homeassistant_adapter import poll_items
...
def beautify(text):
    beautifier = ha_device.get("beautifier", {})
    result = text
        ...
        for item in all_items:
            ...
            for k, v in domain_config_clean.items():
                if k not in discovery_config:
                    if k.endswith("command_topic"):
                        v = v.replace("%mqtt_listen%", settings.mqtt_listen)
                    if k.endswith("_topic") and not k.endswith("command_topic"):
                        e = poll_map.get(v)
                        if e is not None:
                            v = f"{mqtt_base}/{v}"
